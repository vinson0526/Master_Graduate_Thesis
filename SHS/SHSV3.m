function [ out ] = SHSV3( wave_name, out_file)
%%%
%从左至右viterbi译码
%%%
point_number = 4096;
...
%存储当前点概率值，和它的前一帧的频点
viterbi(semi_number_voice, length(T)) = struct('prob',[],'last',[]);
...
for iter = 1 : semi_number_voice
    last_frame(iter) = viterbi(iter, length(T)).prob;
end
local = zeros(length(T), 1);
[~, local(length(T))] = max(last_frame, [], 1);

for iter = length(T) - 1 : -1 : 1
    local(iter) = viterbi(local(iter + 1), iter + 1).last;
end
