...
from prep_data_peaks import load_label_data, load_unlabel_data
...
class DBN(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, n_ins=100,
                 hidden_layers_sizes=[500, 500], n_outs=40):
...
def test_DBN(finetune_lr=0.005, pretraining_epochs=10, # TODO 100+
             pretrain_lr=0.01, k=1, training_epochs=10, # TODO 100+
    ...
    unlabel_datasets = load_unlabel_data(dataset)
    label_datasets = load_label_data(dataset)

    unlabel_train_set = unlabel_datasets [0]
    train_set_x, train_set_y = label_datasets[0]
    #valid_set_x, valid_set_y = label_datasets[1]
    #test_set_x, test_set_y = label_datasets[2]

    # compute number of minibatches for training, validation and testing
    n_train_batches = train_set_x.get_value(borrow=True).shape[0] / batch_size
    n_unsupervised_batches = unlabel_train_set.get_value(borrow=True).shape[0]
    n_unsupervised_batches /= batch_size
    ...
    pretraining_fns = dbn.pretraining_functions(train_set_x=unlabel_train_set,
    ...
    for i in xrange(dbn.n_layers):
        ...
        for epoch in xrange(pretraining_epochs):
            ...
            c = []
            for batch_index in xrange(n_unsupervised_batches):
            ...
            print numpy.mean(c)
        with open(output + '/' + output_file_name + '_layer_' + str(i) + '.pickle', 'w') as f:
            cPickle.dump(dbn, f)
    ...
    train_fn, validate_model, test_model = dbn.build_finetune_functions(
                datasets=label_datasets, batch_size=batch_size,
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if this_validation_loss < best_validation_loss:
                    with open(output + '/' + output_file_name + '.pickle', 'w') as f:
                        cPickle.dump(dbn, f)
